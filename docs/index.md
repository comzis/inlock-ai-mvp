# Inlock AI Knowledge Base

Welcome to the centralized documentation for the Inlock AI MVP infrastructure.

## 📚 Quick Links

- **[Quick Start Guide](../QUICK-START.md)** - Get started quickly
- **[Project Improvements](reports/PROJECT-IMPROVEMENTS-2025-12-25.md)** - Recent improvements and recommendations
- **[Reorganization Plan](reports/REORGANIZATION-PLAN-2025-12-25.md)** - Documentation reorganization status

## 📚 Deployment (Day 2)
Guides for updating the application.
- [Deployment Guide](deployment/DEPLOYMENT.md)
- [Manual Deployment](deployment/MANUAL-DEPLOYMENT.md)
- [Inlock AI Deployment](deployment/INLOCK-AI-DEPLOYMENT.md)
- [Secret Management](guides/SECRET-MANAGEMENT.md)
- [Adding New Services](guides/ADDING-NEW-SERVICE.md)

## 🛠 Provisioning (Day 0)
Guides for initial server setup (inlock-infra).
- [Server Provisioning Guide](guides/SERVER-PROVISIONING-GUIDE.md) (formerly 10-10 Security)
- [Firewall Setup (Ansible)](guides/PROVISIONING-FIREWALL-ANSIBLE.md)
- [Infra Repo Publishing](guides/INFRA-REPO-PUBLISH.md)

## 🔧 Service Guides
How-to guides for specific services.
- [Mailcow Deployment](deployment/MAILCOW-DEPLOYMENT.md)
- [Portainer Access](services/portainer/PORTAINER-ACCESS.md)
- [Cockpit Setup](services/cockpit/)
- [Monitoring Setup](services/monitoring/MONITORING.md)
- [Auth0 Integration](services/auth0/)
- [n8n Workflows](services/n8n/)

## 🏗 Architecture
High-level design documentation.
- [DevOps Workflow (Diagram)](architecture/DEVOPS-WORKFLOW.md)
- [Container Structure](architecture/CONTAINER-STRUCTURE.md) (Service stacks & Volumes)
- [Network Layout](architecture/NETWORK-LAYOUT.md) (Isolation & Security)
- [Server Structure Analysis](architecture/SERVER-STRUCTURE-ANALYSIS.md)
- [Infra Notes](architecture/INFRA-NOTES.md)

## 🔒 Security
Security documentation and guides.
- [Security Audit](security/SECURITY-AUDIT-COMPREHENSIVE-2025-12-24.md)
- [Security Posture](security/SECURITY-POSTURE-2025-12-24.md)
- [Access Control Validation](security/ACCESS-CONTROL-VALIDATION.md)
- [Cloudflare IP Allowlist](security/CLOUDFLARE-IP-ALLOWLIST.md)
- [Port Restrictions](security/PORT-RESTRICTION-SUMMARY.md)
- [Ingress Hardening](security/INGRESS-HARDENING.md)

## 📖 Reference
Technical specifications and cheat sheets.
- [Auth0 API Reference](reference/AUTH0-API-REFERENCE.md)
- [Auth0 Quick Reference](reference/AUTH0-QUICK-REFERENCE.md)
- [Quick Reference Card](reference/QUICK-REFERENCE.md)

## 📊 Reports
Status reports, incident logs, and operational documentation.
- [Development Status](reports/DEVELOPMENT-STATUS-UPDATE.md)
- [Project Review (Cursor Rules)](reports/PROJECT-REVIEW-2025-12-28.md)
- [Execution Reports](reports/EXECUTION-REPORT-2025-12-13.md)
- [Verification Reports](reports/VERIFICATION-REPORT.md)
- [Incident Reports](reports/incidents/)
- [SWARM Reports](reports/SWARM-*.md)

## 📝 Guides
Day-to-day operational guides and how-tos.
- [Automation Scripts](guides/AUTOMATION-SCRIPTS.md)
- [Orphan Container Cleanup](guides/ORPHAN-CONTAINER-CLEANUP.md)
- [Run Diagnostics](guides/RUN-DIAGNOSTICS.md)
- [Workflow Best Practices](guides/WORKFLOW-BEST-PRACTICES.md)
- [Inlock Content Management](guides/INLOCK-CONTENT-MANAGEMENT.md)
- [Quick Action Checklist](guides/QUICK-ACTION-CHECKLIST.md)
- [Website Launch Checklist](guides/WEBSITE-LAUNCH-CHECKLIST.md)

## 🗄 Archive
Historical status reports, incident logs, and checklists.
- [Deployment Status History](deployment/DEPLOYMENT-STATUS-2025-12-11.md)
- [Strike Team Reports](reports/incidents/STRIKE-TEAM-FINAL-SUMMARY.md)

---
*Last Updated: 2025-12-29*  
*Documentation reorganized per `.cursorrules` structure*
