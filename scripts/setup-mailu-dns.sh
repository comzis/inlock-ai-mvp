...
SERVER_IP="${SERVER_IP:-}"
if [ -z "$SERVER_IP" ]; then
    echo "⚠️  SERVER_IP not set. Attempting to detect..."
...
if [ -z "$SERVER_IPV6" ]; then
    read -p "Enter server IPv6 address (optional, press Enter to skip): " SERVER_IPV6
fi

# Get Cloudflare Zone ID
echo "📡 Fetching Cloudflare zone information for $DOMAIN..."
...
create_or_update_record() {
    ...
    echo ""
    echo "📝 Processing $record_type record: $record_name"
    ...
    RECORD_ID=$(echo "$EXISTING_RECORDS" | grep -o '"id":"[^"]*"' | head -1 | cut -d'"' -f4)
    ...
    if [ -n "$RECORD_ID" ]; then
        echo "   ℹ️  Record exists, updating..."
...
echo ""
read -p "Press Enter to continue or 's' to skip DKIM setup instructions..."

# Summary
echo ""
echo "✅ DNS Configuration Complete!"
echo ""
echo "📋 Summary of records created:"
echo "   - A record: mail.$DOMAIN → $SERVER_IP"
[ -n "$SERVER_IPV6" ] && echo "   - AAAA record: mail.$DOMAIN → $SERVER_IPV6"
echo "   - MX record: $DOMAIN → mail.$DOMAIN (priority 10)"
echo "   - SPF record: $DOMAIN → $SPF_CONTENT"
echo "   - DMARC record: _dmarc.$DOMAIN → $DMARC_CONTENT"
